...
function dbInit() {
  dropCollection('contests').then(() => {
    dropCollection('names').then(() => {
      populateContests()
        .then(response => {
...
function dropCollection(col) {
  return mdb.listCollections({ name: col }, { nameOnly: true }).toArray()
    .then(items => {
      if (items.length > 0) {
        return mdb.collection(col).drop();
      }
    })
    .catch(console.error);
}
...
function populateContests() {
  return mdb.collection('contests').insertMany([
...
function populateNames() {
  return mdb.collection('names').insertMany([
